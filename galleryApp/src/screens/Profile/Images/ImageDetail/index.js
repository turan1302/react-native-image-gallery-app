...
export default class ImageDetail extends Component {
  ...
  componentDidMount() {
    ...
    if (img_image) {
      this.setState({ imageUri: img_image });
    }
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.route.params?.img_image !== route.params?.img_image) {
      ...
      this.setState({ imageUri: route.params.img_image });
    }
  }
  ...
  getImage = async () => {
    ...
    RestClient.getRequest(AppUrl.images_edit + `/${route.params.img_id}`, {
      headers: {
  ...
  _handleSubmit = async (values, { resetForm, setSubmitting, setFieldValue }) => {
    ...
    RestClient.postRequest(AppUrl.images_update + `/${image.img_id}`, formData, {
      headers: {
        "Authorization": "Bearer " + token,
              ...
              {({
                ...
                <View>
                  ...
                  {(touched.img_image && errors.img_image) &&
                    <Text style={[styles.error_text, { margin: 10 }]}>{errors.img_image}</Text>}
                  ...
                  <View style={styles.form_area}>
                    ...
                    <View style={styles.desc_area}>
                      ...
                      <View style={styles.desc_input_area}>
                        <TextInput multiline style={styles.desc_input}
                                   value={values.img_desc} onChangeText={handleChange("img_desc")}
                                   onBlur={handleBlur("img_desc")}
                                   placeholderTextColor={"#000"}
